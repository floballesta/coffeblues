...
class Upload{
    ...
    private $taille;
    /*
    $extensions array comprenant toutes les extensions $chemin vers le répertoire de stockage des fichiers $taille taille max du fichier en ko
            ...
            if(!empty($_FILES[$data]['name'])){
                if( !in_array( substr(strrchr($_FILES[$data]['name'], '.'), 1), $this->extensions) ){ 
                    ...
                    if( file_exists($_FILES[$data]['tmp_name'])&& (filesize($_FILES[$data]['tmp_name'])) > $this->taille){
                        ...
                        // enlever les accents
                        $fichier['nom']=strtr($fichier['nom'],'ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèé êëìíîïðòóôõöùúûüýÿ','AAAAAACEEEEIIIIOOOOOUUUUYaaaaaaceeeeiiiioooooouuuuyy');
